...
bool AudioBase::initI2SSpeakOrMic(int mode)
{ // Init I2S.  初始化I2S
...
void AudioBase::setup()
{
  ...
  microphoneData = new uint8_t[MAX_DATA_SIZE];
...
void AudioBase::loop()
{
  ...
  if (pos.y > 240)
  {
    if (pos.x < 109)
    {
      ...
      while (1)
      {
        i2s_read(Speak_I2S_NUMBER, (char *)(microphoneData + data_offset),
...
void AudioBase::preStateChange(int s)
{
